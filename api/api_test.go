...
func TestGzip(t *testing.T) {
	...
	handler := http.HandlerFunc(GzipHandler(ServeServerVer))
...
func TestServeIndexNoDir(t *testing.T) {
	...
	addNode("testing", &Node{"0.0.0.0", "0.0.0", time.Now().Format(time.RFC850), true, false})
...
func TestServeServerVer(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	handler := http.HandlerFunc(ServeServerVer)
...
func TestServeSync(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	handler := http.HandlerFunc(ServeSync)
...
func TestListNodes(t *testing.T) {
	recorder := httptest.NewRecorder()
	handler := http.HandlerFunc(ListNodes)

	CurrentNodes = nil
	addNode("testing0", &Node{"0.0.0.0", "0.0.0", time.Now().Format(time.RFC850), true, false})
	addNode("testing1", &Node{"0.0.0.1", "0.0.0", time.Now().Format(time.RFC850), true, false})
	addNode("testing2", &Node{"0.0.0.2", "0.0.0", time.Now().Format(time.RFC850), true, false})
	addNode("testing3", &Node{"0.0.0.3", "0.0.0", time.Now().Format(time.RFC850), true, false})
	...
	if reflect.DeepEqual(response, CurrentNodes) == false {
...
func TestIdentify(t *testing.T) {
	recorder := httptest.NewRecorder()
	handler := http.HandlerFunc(Identify)
...
func TestHeartBeat(t *testing.T) {
	...
	if getNodeByUUID("testing").Synced != true {
